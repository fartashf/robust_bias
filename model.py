...
@jax.jit
def linear_model_loss_l2(w, x, y):
  """Returns the loss given an input and its ground-truth label."""
  z = w.T @ x
  loss = ((y - z)**2).sum()
  return loss
...
def two_linear_model_predict(w0, w1, x):
  ...
  """
  # No bias. The implicit bias result does not include bias.
  z = w1.T @ (w0.T @ x)
  return 2. * (z > 0) - 1
...
def circ_1d_conv(w, x, do_conv=False):
  ...
  """
  sign = +1 if do_conv else -1
  rcvvi = lambda w, x, i: w.T @ jnp.roll(x, sign*i, axis=0)  # [a],[a],[] -> []
...
def conv_linear_model_init_param(dim, nlayers, r, rng_key):
  """Creates initial parameters for a linear model."""
  params = []
  # rng_subkey = jax.random.split(rng_key, nlayers)
  rng_subkey = jax.random.split(rng_key, nlayers - 1)
  for i in range(nlayers - 1):
    params += [init_normalized_params((dim,), r, rng_subkey[i], False)]
  params += [jnp.zeros((dim, 1))]
  # params += [init_normalized_params((dim,), r, rng_subkey[nlayers-1], False)]
  return params
...
@jax.jit
def conv_linear_model_logistic_loss(w, x, y):
  """Returns the loss given an input and its ground-truth label."""
  z = x
  for i in range(len(w) - 1):
    z = circ_1d_conv(w[i], z)
...
@jax.jit
def conv_linear_model_exp_loss(w, x, y):
  """Returns the loss given an input and its ground-truth label."""
  z = x
  for i in range(len(w) - 1):
    z = circ_1d_conv(w[i], z)
  z = w[-1].T @ z
  loss = jnp.exp(-y * z).mean()
  return loss


def conv_linear_model_linearize_param(params):
  return conv_linear_model_linearize_param_flip_at_one(params)


def conv_linear_model_linearize_param_noflip(params):
  """Computes the product of parameters for convolving with do_conv=True."""
  wl = params[-1]
...
def conv_linear_model_linearize_param_flip_at_zero(params):
  """Computes the product of parameters flipping the entire weight vector."""
  wl = params[0]
  for i in range(1, len(params)-1):
    wl = circ_1d_conv(params[i], wl)
  wl = jnp.flip(wl, 0)
  wl = circ_1d_conv(params[-1], wl)
  wl = jnp.flip(wl, 0)
  return wl
