  ...
  for i in range(len(logdata)):
    if not isinstance(summary_tag, list):
      ...
      if summary_tag in logdata[i]:
        d = np.array(logdata[i][summary_tag], dtype=object).T
    else:
      d = []
...
def plot_one_summary_tag(data,
  ...
  Args:
    ...
    global_legends: A list of strings of legends globally plotted across
      multiple plots. Colors will match the order in this list.
  """
  ...
  for i in range(len(data)):
    ...
    if global_legends is not None:
      idx = global_legends.index(legends[i])
    plot_kwargs = {
        'linestyle': style[idx // len(color)],
        'color': color[idx % len(color)],
        'linewidth': 2,
        'marker': marker[idx % len(color)],
        'markersize': 15,
    }
  ...
  for ext in ['pdf', 'png']:
    path = '{}/{}_nolegend.{}'.format(fig_dir, plot_texts['filename'],
                                              ext)
    with open(path, 'wb') as file:
      plt.savefig(file, format=ext, dpi=100, bbox_inches='tight')
  ...
  # Save without legends
  for ext in ['pdf', 'png']:
    path = '{}/{}_notitle.{}'.format(fig_dir, plot_texts['filename'],
                                              ext)
    with open(path, 'wb') as file:
      plt.savefig(file, format=ext, dpi=100, bbox_inches='tight')
  ...
  # Save global legends
  if global_legends is None:
    global_legends = legends
  ...
  for i in range(len(global_legends)):
    ...
    plot_f(0, 0, label=global_legends[i], figure=fig, **plot_kwargs)
  fig2 = plt.figure()
  ...
  bbox = legend.get_window_extent().transformed(
    fig3.dpi_scale_trans.inverted())
  for ext in ['pdf', 'png']:
    path = '{}/{}_legend.{}'.format(fig_dir, plot_texts['filename'], ext)
    with open(path, 'wb') as file:
      plt.savefig(file, format=ext, dpi=100, bbox_inches=bbox)
  plt.close(fig)
  plt.close(fig2)
  plt.close(fig3)
