...
def evaluate_adversarial_risk(data, predict_f, loss_adv_f, dloss_adv_dx,
  ...
  adv_risk = []
  adv_eps = []
  for i in list(jnp.arange(0, 1.05, 0.05))+list(jnp.arange(0.45, .5, .005)):
    ...
    x_adv_multi = adversarial.find_adversarial_samples_multi_attack(
        data, loss_adv_f, dloss_adv_dx, linearize_f,
...
def main_with_config(config):
  """Function that loads and initializes then calls train."""
