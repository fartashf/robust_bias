...
def gradient_descent_line_search_step(
  ...
  value_and_grad_f = jax.jit(jax.value_and_grad(loss_with_data_f))
  ...
  # Wolfe conditions are stronger than FISTA conditions because the gradient
  # is reevaluated at candidate
...
def fista_step(data, loss_and_prox_op, model_param, options):
  ...
  value_and_grad_f = jax.jit(jax.value_and_grad(fun_f))
  x, unravel_fn = ravel_pytree(model_param)
  y = options.get('y', x)
