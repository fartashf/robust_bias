...
def get_prox_op(norm_type):
  ...
  if norm_type == 'l1':
    ...
    prox_op = lambda v, lam: v * jnp.maximum(0, 1 - lam / jnp.sqrt(
        jnp.sum(v**2)))
  elif norm_type == 'linf':
    prox_op = lambda v, lam: v - lam * l1_unit_projection(v / lam)
  else:
    dual_norm = norm_type_dual(norm_type)
    prox_op = lambda v, lam: v - lam * norm_projection(
        v / lam, dual_norm, eps=1)
  return prox_op


def norm_shrink(delta, norm_type, lam):
  delta_norm = jax.vmap(lambda x: norm_f(x, norm_type), (1,), 0)(delta)
  norm_proj = jax.vmap(lambda x, n: norm_projection(x, norm_type, n),
                       (1, 0), 1)
  delta_shrink = norm_proj(delta, delta_norm*lam)
  return delta_shrink
